...
function defaultMetaTags(type: string = 'articles') {
  const defaultTags = _defaultMetaTags();
    ...
    this.data$ = urlParams(this.route).pipe(
      ...
      map(data => {
        ...
        if (data instanceof Array) {
          data.map((item: Article) => this.adjustArticle(item)) as Article[];
        } else if (!data.error) { data = this.adjustArticle(data as Article); }

        let metaTags;
        if (!(data instanceof Array)) {
          const defaultTags = defaultMetaTags(this.params.type);
...
*/
    ...
    const comp = this.elementRef.nativeElement;
    if (this.dev) {
